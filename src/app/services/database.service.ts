...
export class FirestoreService {
  ...
  async clearGroup(groupID) {
    let path = `users/${this.authService.uid}/groups/${groupID}`;
    // this.deleteCollection(path + "/items");
    // this.deleteCollection(path + "/columns");
    this.httpService.get(`${BASE_URL}/firestore-deleteCollection?path=${path}/items`)
    await this.addColumn(groupID, "Name");
    await this.addColumn(groupID, "Attribute");
    return;
  }
